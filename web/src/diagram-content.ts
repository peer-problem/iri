...
export const diagramContent: Record<IllustrationKind, { title: string; modes: string[]; notes: string[]; labels: DiagramLabel[] }> = {
  ...
  inspection: {
    ...
    notes: ["입력 허용 → 어댑터 답변 생성 → 출력 허용 → 음성 합성 순서입니다.", "입력 검사에서 안내가 필요하다고 판단하면 일반 답변 생성을 건너뛰고 정책에 따른 안내를 반환합니다.", "생성한 답변을 출력 검사에서 차단하면 해당 답변을 전달하지 않고 정책에 따른 안내로 전환합니다.", "Kanana를 사용할 수 없으면 Luna가 입력 검사부터 생성과 출력 검사까지 다시 수행합니다."],
    labels: [
      ...
      { title: "Luna 대체 경로", detail: "입력 검사 → 생성 → 출력 검사", at: [1.4, -2, 0], tone: "blue" },
    ],
  },
  age: {
