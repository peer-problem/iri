...
`npm test`는 음성 신호 보정과 애니메이션 완화 수식, 응답 제공자 표시를 확인한다. 또한 네트워크 경계에서 나뉜 SSE 데이터의 재조립과 완료 여부 및 무결성을 검사한다. `npm run build`는 TypeScript와 프로덕션 번들을 검증한다. STT부터 TTS까지의 서버 계약은 저장소 루트에서 실행하는 Python API 테스트로 확인하며, 실제 기기의 마이크와 스피커 동작은 별도로 검증해야 한다.
