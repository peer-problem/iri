...
api/
  app/               FastAPI와 모델 연결
    operations/      데이터 검수, 평가, 학습과 Runpod 관리 도구
  configs/           모델 후보와 연령별 대응 기준
  data/              개발 질문과 출처 기록. raw/와 prepared/는 Git 제외
  tests/             GPU 없이 실행하는 자동 테스트
  pyproject.toml     Python 의존성과 도구 설정
  uv.lock            고정한 Python 의존성
  requirements-*     GPU 추론과 학습 의존성
  .venv/             로컬 가상환경. Git 제외
  runs/              실행 기록. Git 제외
  artifacts/         전송 묶음과 학습 결과. Git 제외
.agents/             계획과 작업 문서. Git 제외
.env                 공통 환경 설정. Git 제외
AGENTS.md            로컬 작업 규칙. Git 제외
README.md            팀 공통 안내
```

화면 구현을 시작하면 `dashboard/`를 추가한다. GitHub Actions는 사용하지 않으며 검사는 로컬에서 실행한다.

## 로컬 시작

Python 3.12와 [uv](https://docs.astral.sh/uv/)를 사용한다. 아래 명령과 이후 실행 예시는 `api/` 안에서 실행한다. 환경 설정은 저장소 루트 `.env`를 읽는다.

```bash
cd api
uv sync --frozen
uv run python -m app.operations.init_local
uv run pytest
uv run python -m app.operations.data --phase-one
uv run uvicorn app.app:app --host 127.0.0.1 --port 8000 --workers 1 --no-access-log
```
...
저장소를 공유받으면 위의 로컬 시작 명령부터 실행한다. API 개발과 자동 테스트에는 GPU나 유료 API 키가 필요하지 않다. 각자 `app.operations.init_local`로 개발용 키를 생성하고 개인 `.env`를 사용한다.

현재 구현한 범위는 인증된 텍스트 API, 안전성 검사 흐름, 음성 업로드 API와 평가 및 QLoRA 실행 도구다. 자동 테스트는 79개이며 실제 모델 성능과 아동 적합성을 검증한 결과는 아니다. Runpod 연결과 실제 모델 비교는 아직 진행하지 않았다.
...
| 데이터 검수 | api/data/, api/app/operations/candidate_review.py | 연령별로 사람이 승인한 질문과 답변 |
| GPU 실행과 평가 | api/app/operations/ | Kanana와 Gemma 응답 및 지연 비교 |
...
로컬에서 `uv run python -m app.operations.resolve_model kanana`로 모델 리비전을 확인한다. 출력된 MODEL_PROFILE과 MODEL_REVISION을 로컬 및 GPU의 `.env`에 반영하고 MODEL_API_KEY도 일치시킨다. GPU에는 HF_TOKEN을 별도로 설정한다.
...
```bash
uv run python -m app.operations.doctor
uv run python -m app.operations.doctor --online
uv run python -m app.operations.bundle --output artifacts/runpod-source.zip
```
...
uv run python -m app.operations.source_square
uv run python -m app.operations.candidate_review export data/raw/square/candidates.jsonl --output data/raw/square/review.csv

# CSV 검수 후 원본을 보존하면서 반영
uv run python -m app.operations.candidate_review import data/raw/square/candidates.jsonl --edits data/raw/square/review.csv --output data/raw/square/reviewed.jsonl
uv run python -m app.operations.prepare_training data/raw/square/reviewed.jsonl
```
...
```bash
uv run python -m app.operations.review export runs/KANANA_RUN runs/GEMMA_RUN --output runs/blind-review
uv run python -m app.operations.review aggregate runs/blind-review runs/blind-review/reviewer_a/ratings.csv runs/blind-review/reviewer_b/ratings.csv
```
...
.venv-training/bin/python -m app.operations.train --smoke --output artifacts/first-smoke
...
uv run python -m app.operations.runpod_guard status
uv run python -m app.operations.runpod_guard arm --pod-id POD_ID --minutes 60
# 실제 작업 중에만 3분 이내 간격으로 갱신한다. 반복 갱신을 자동화하지 않는다.
uv run python -m app.operations.runpod_guard heartbeat --pod-id POD_ID

# Pod: 로컬 컴퓨터와 별도로 동작하는 제한 시간. GPU 작업 전에 실행한다.
python -m app.operations.pod_deadline --minutes 60
# 학습할 때는 위 명령에 --training-output /workspace/프로젝트/api/artifacts/실험명 추가

# 작업 종료 직후 로컬에서 실행한다. 타이머가 끝날 때까지 기다리지 않는다.
uv run python -m app.operations.runpod_guard stop --pod-id POD_ID
uv run python -m app.operations.runpod_guard status
```
