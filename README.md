...
TTS는 서버에서 고정한 `coral` 음성을 속도 `0.95`로 사용한다. 어린아이에게 천천히 다정하게 말하되 음높이와 속도를 일정하게 유지하고, 과장된 감정 표현이나 속삭임을 피하도록 모든 합성 요청에 같은 지시를 적용한다. 로컬에서 비교 실험할 때만 `.keys/.env`의 `TTS_SPEED` 또는 `TTS_INSTRUCTIONS`를 변경한다.
