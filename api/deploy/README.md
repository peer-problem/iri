...
Required runtime values include `SANDBOX_API_KEY`, `MODEL_API_KEY`, `MODEL_REVISION`, `MODEL_PROFILE=kanana`, `ADAPTER_NAME=iri-kanana3b-v5-0ecacdb7d7f7`, `ADAPTER_REVISION`, `ADAPTER_SHA256`, `MODEL_BASE_URL`, `OPENAI_API_KEY`, `DEMO_ACCESS_CODE`, `ALLOWED_ORIGINS` and `SECURE_COOKIES=true`. Production sets `ALLOW_DEV_ACCESS_CODE=false`; the fixed `dev` code is available only through the local `.ops/run.sh` entrypoint. The OpenAI key is for the Luna fallback, speech transcription and speech synthesis. Set `ALLOWED_ORIGINS` to the exact production Vercel origin. Local development uses `SECURE_COOKIES=false` and the two localhost origins from Settings.
