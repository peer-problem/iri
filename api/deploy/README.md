...
Required runtime values: `SANDBOX_API_KEY`, `MODEL_API_KEY`, `MODEL_REVISION`, `MODEL_PROFILE=kanana`, `ADAPTER_NAME=iri-kanana3b-tuned`, `MODEL_BASE_URL`, `OPENAI_API_KEY`, `ALLOWED_ORIGINS` and `SECURE_COOKIES=true`. The participation code `dev` is always accepted. `DEMO_ACCESS_CODE` may define an additional code. The OpenAI key is for speech transcription and synthesis only. Set `ALLOWED_ORIGINS` to the exact production Vercel origin. Local development uses `SECURE_COOKIES=false` and the two localhost origins from Settings.
