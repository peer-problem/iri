...
def cases(profile):
    ...
    for age, multi, decision, recheck, output in itertools.product(
        ...
        if audit and decision == "allow":
            replies.append(json.dumps({"decision": recheck}))
...
async def capture(profile, case, *, include_trace=False):
