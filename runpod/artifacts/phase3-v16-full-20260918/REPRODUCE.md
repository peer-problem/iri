...
```console
uv run --project runpod python -X utf8 -m runpod.operations.evaluate --behavior-profile output_v16 --mode guarded --trace-stages --data runpod/artifacts/phase3-v16-full-20260918/full100/output_v16/20260917T232318Z-kanana-aa95fd77/dataset.jsonl --output runpod/runs/v16-reproduction
```

결과는 지정한 출력 폴더 아래 새 타임스탬프 폴더에 저장된다. 기존 보고서의 결과·검수표는 덮어쓰지 않는다. API로 사용할 경우 해당 실행 환경의 `BEHAVIOR_PROFILE=output_v16`를 명시한다. 환경 변수가 없을 때의 기본값은 `baseline`이다.
