...
```console
uv run --project runpod python -X utf8 -m runpod.operations.evaluate --behavior-profile boundary_v25 --mode guarded --trace-stages --data runpod/artifacts/phase3-v25-full-20260918/full100/boundary_v25/20260918T001747Z-kanana-0c7e4287/dataset.jsonl --output runpod/runs/v25-reproduction
```

결과는 지정한 출력 폴더 아래 새 타임스탬프 폴더에 저장된다. 기존 보고서의 결과·검수표는 덮어쓰지 않는다. API로 사용할 경우 해당 실행 환경의 `BEHAVIOR_PROFILE=boundary_v25`를 명시한다. 환경 변수가 없을 때의 기본값은 `baseline`이다.

## 검증 범위

- 공개 구현을 이 버전의 **원래 동결 소스 ZIP**과 비교했다. 출처 해시는 [공유 목록](share-manifest.json)에 있다.
- 두 연령대, 단일·다중 대화, 입력·출력 판정 및 오류 경로 43개에서 모델 요청과 반환 행동·오류·단계 기록이 동일했다.
