...
결과는 지정한 출력 폴더 아래 새 타임스탬프 폴더에 저장된다. 기존 보고서의 결과·검수표는 덮어쓰지 않는다. API로 사용할 경우 해당 실행 환경의 `BEHAVIOR_PROFILE=legacy_harm_v63`를 명시한다. 환경 변수가 없을 때의 기본값은 `baseline`이다.
